...
const ALLOWED_KEYS = new Set(['home_feed_rank_v2_enabled', 'explore_rank_v2_enabled'])
...
export async function POST(request: Request) {
  ...
  if (!ALLOWED_KEYS.has(key)) {
    return NextResponse.json({ error: 'Invalid ranking flag key' }, { status: 400 })
  }
  ...
  const { data: existingIdempotent } = await auth.supabase
    ...
    .select('id')
    .eq('action_type', 'RANKING_FLAG_CHANGE')
    .eq('target_id', key)
  ...
  const { data: result, error } = await auth.supabase.rpc('admin_update_ranking_feature_flag', {
    p_key: key,
    p_enabled: enabled,
  ...
  await auth.supabase.from('admin_audit_logs').insert({
    ...
    action_type: 'RANKING_FLAG_CHANGE',
    target_id: key,
    details: {
      message: `Ranking flag ${key} updated.`,
      key,
      enabled,
      rolloutPercent,
      reason,
